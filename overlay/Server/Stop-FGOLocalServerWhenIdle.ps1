...
$frontends = @((Join-Path $installRoot 'FGOLocalPlatform.exe'), (Join-Path $installRoot 'FGOLocalPlatform-独立版.exe'), (Join-Path $installRoot 'FGOAC scooby.exe'), (Join-Path $installRoot 'FGOA scooby.exe'))
...
try {
    ...
    while ($true) {
        $processes = @(Get-CimInstance Win32_Process -Filter "Name='ago.exe' OR Name='FGOLocalPlatform.exe' OR Name='FGOLocalPlatform-独立版.exe' OR Name='FGOAC scooby.exe' OR Name='FGOA scooby.exe'" -OperationTimeoutSec 5)
