...
internal static class DisplayMonitor
{
	...
	internal static IReadOnlyList<Entry> GetConnected()
	{
		...
		EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, delegate(IntPtr monitor, IntPtr _, IntPtr _, IntPtr _)
		{
			...
			{
				string value = info.Device.Replace("\\\\.\\DISPLAY", "屏幕 ");
				entries.Add(new Entry(info.Device, $"{value}{(((info.Flags & 1) != 0) ? "（主屏）" : "")} · {info.Monitor.Right - info.Monitor.Left}×{info.Monitor.Bottom - info.Monitor.Top}"));
			}
			return true;
		}, IntPtr.Zero);
		return entries;
